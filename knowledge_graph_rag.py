...
DEFAULT_LLM_MODEL = os.environ.get('LLM_MODEL', 'llama3.2')
...
def generate_answer_with_citations(
    ...
    for entity in initial_results:
        context_key = (entity['name'], entity['source'], entity['chunk'])
...
def main():
    ...
    model_options = ["llama3.2", "mistral", "phi3"]
    if DEFAULT_LLM_MODEL not in model_options:
        model_options.insert(0, DEFAULT_LLM_MODEL)
    
    neo4j_uri = st.sidebar.text_input("Neo4j URI", DEFAULT_NEO4J_URI)
    neo4j_user = st.sidebar.text_input("Neo4j User", DEFAULT_NEO4J_USER)
    neo4j_password = st.sidebar.text_input("Neo4j Password", type="password", value=DEFAULT_NEO4J_PASSWORD)
    llm_model = st.sidebar.selectbox("LLM Model", model_options, index=model_options.index(DEFAULT_LLM_MODEL))
